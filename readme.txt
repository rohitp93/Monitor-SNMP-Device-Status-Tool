...
Assignment4
-----------
The objective of this assignment is to find the sysuptime for about 150 devices using non-blocking SNMP calls. 
Each device is probed every 30 seconds. The results are presented through a web dashboard. The status of each device is shown using a variation of red color.
For one missed request, the status color is #FFEEEE(light red). The more requests the device misses, the more darker shade of red the status becomes. 
After 30 missed requests, the status color becomes #FF0000(red). If a device responds, the status is normalized to #FFFFFF(white).
...
3. To run the backend, run the perl script "backend.pl" in the terminal with the command "perl backend.pl".
...
4. Now, open a web browser and type the following URL:
   http://localhost/et2536-ropo15/assignment4/
